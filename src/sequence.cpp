...
void Sequence::parse_label(string label, bool is_user_fasta) {
	...
	vector<string> toks;
	Tokenize(label, toks, SEQ_FIELD_SEP);
	...
	// case 2: when reading in an alignment with phlawd-formatted labels
		...
		set_sqlite_id(atoi(toks[4].c_str())); 		// convert sqlite id to integer
		set_is_aligned(!!atoi(toks[5].c_str()));	// convert is_aligned value ('1' or '0') to boolean
